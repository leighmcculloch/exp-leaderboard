...
class LeaderboardApp {
    constructor() {
        ...
        setInterval(() => this.refreshAllContractData(), 30000);
    }
    ...
    saveContractsToStorage() {
        try {
            sessionStorage.setItem(this.sessionStorageKey, JSON.stringify(this.contracts));
        } catch (error) {
    ...
    addContract(address) {
        ...
        if (!this.isValidContractAddress(address)) {
            alert('Please enter a valid Stellar contract address (56 characters, A-Z and 2-7)');
            return;
        }
    ...
    removeContract(address) {
        this.contracts = this.contracts.filter(contract => contract.address !== address);
        this.saveContractsToStorage();
        this.renderTable();
    }
    ...
    updateContractStatusInTable(address, state, status = null) {
        const contractIndex = this.contracts.findIndex(c => c.address === address);
        if (contractIndex === -1) return;

        const metrics = ['deployed', 'build-verified', 'minted', 'soroswap-pair', 'soroswap-swapped'];
        
        metrics.forEach(metric => {
            const cell = document.querySelector(`[data-metric="${metric}"] .status-cell[data-contract="${address}"]`);
            if (!cell) return;
            ...
            if (state === 'loading') {
                ...
                const metricKey = metric.replace('-', '_').replace('_', '').toLowerCase();
                let statusKey;
